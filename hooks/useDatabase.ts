...
export function useDatabase() {
  ...
  const insertSession = useCallback(async (session: ChatSession): Promise<void> => {
  ...
  const loadSessionsFromJSON = useCallback(async (jsonData: ChatSession[]): Promise<number> => {
    if (!db) throw new Error('Database not initialized');
  ...
  const getDatabaseStats = useCallback((): { totalSessions: number; dateRange: { min: string; max: string } } => {
